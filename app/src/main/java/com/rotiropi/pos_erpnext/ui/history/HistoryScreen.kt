...
fun HistoryScreen(state: HistoryUiState, onQueryChanged: (String) -> Unit, onSaleSelected: (String) -> Unit, onLoadMore: () -> Unit, onRetry: () -> Unit, modifier: Modifier = Modifier) {
    when (state) {
        HistoryUiState.Unavailable -> Message(stringResource(R.string.history_unavailable), modifier)
        is HistoryUiState.Content -> LazyColumn(modifier.fillMaxSize().testTag("history-screen"), contentPadding = androidx.compose.foundation.layout.PaddingValues(PosDimensions.screenPadding), verticalArrangement = Arrangement.spacedBy(PosDimensions.sectionSpacing)) {
            item { Text(stringResource(R.string.history_title), style = MaterialTheme.typography.headlineMedium, modifier = Modifier.semantics { heading() }) }
            item { OutlinedTextField(state.query, onQueryChanged, label = { Text(stringResource(R.string.history_search_label)) }, singleLine = true, modifier = Modifier.fillMaxWidth().testTag("history-search")) }
            if (state.loading && state.sales.isEmpty()) item { CircularProgressIndicator() }
            if (!state.loading && state.sales.isEmpty() && state.error == null) item { Text(stringResource(R.string.history_empty)) }
            items(state.sales, key = { it.name }) { sale -> SaleRow(sale, onSaleSelected) }
            if (state.error != null) item { Column { Text(state.error, color = MaterialTheme.colorScheme.error); Button(onRetry, Modifier.heightIn(min = PosDimensions.touchTarget)) { Text(stringResource(R.string.action_retry)) } } }
            if (state.hasMore) item { Button(onLoadMore, Modifier.fillMaxWidth().heightIn(min = PosDimensions.touchTarget).testTag("history-load-more")) { Text(stringResource(R.string.action_load_more)) } }
        }
    }
}
